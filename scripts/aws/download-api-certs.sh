...
# Download foobar client cert and key from SSM
CERT_PATH="/ndr/${PERSISTENT_WORKSPACE}/foobar_client_cert"
CERT_KEY="/ndr/${PERSISTENT_WORKSPACE}/foobar_client_key"

aws ssm get-parameter --name "${CERT_PATH}" --with-decryption | jq -r '.Parameter.Value' > "${LOCAL_CERT_DIR}/foobar_client.crt"
aws ssm get-parameter --name "${CERT_KEY}" --with-decryption | jq -r '.Parameter.Value' > "${LOCAL_CERT_DIR}/foobar_client.key"
...
# Verify foobar cert against CA
openssl verify -CAfile "${LOCAL_CERT_DIR}/cacert.pem" "${LOCAL_CERT_DIR}/foobar_client.crt"
# shellcheck disable=SC2181 # - Ignored Check exit code directly as required nested ifs to work with above logic
if [[ $? -eq 0 ]]; then
    echo "The foobar cert matches the Truststore PEM file"
else
    echo "The foobar cert doesn't match the Truststore PEM file"
    exit 1
fi