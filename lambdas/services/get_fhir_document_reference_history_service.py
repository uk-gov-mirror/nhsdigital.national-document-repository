...
class GetFhirDocumentReferenceHistoryService:
    ...
    def get_document_reference_history(
        ...
        query_filter = patient_nhs_number_filter(DynamoQueryFilterBuilder(), nhs_number)

        s3_file_key = self.get_s3_file_key(document_id)
    ...
    def get_item(self, document_id: str) -> str:
        item = self.dynamo_service.query_table(
            table_name=self.lg_table,
            search_key="ID",
            search_condition=document_id,
            query_filter=FinalStatusFilter,
        )
    ...
    def get_s3_file_key(self, document_id: str) -> str:
