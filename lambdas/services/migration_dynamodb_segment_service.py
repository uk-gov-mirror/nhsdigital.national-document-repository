...
class MigrationDynamoDBSegmentService:
    def __init__(self):
        ...
        if not self.bucket_name:
            raise ValueError(
                "MIGRATION_SEGMENT_BUCKET_NAME environment variable is required"
            )
    ...
    def segment(self, id: str, total_segments: int) -> dict:
        try:
            ...
            extras = {
                ...
                "awsErrorCode": aws_error.response.get("Error", {}).get(
                    "Code", "Unknown"
                ),
            }
            logger.error(
                f"AWS error in migration_dynamodb_segment_service: {aws_error}",
                extra=extras,
                exc_info=True,
            )
            raise
            ...
            logger.error(
                f"Exception in migration_dynamodb_segment_service: {e}",
                extra=extras,
                exc_info=True,
            )
            raise
