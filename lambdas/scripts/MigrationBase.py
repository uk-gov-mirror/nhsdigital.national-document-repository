...
class MigrationBase(ABC):
    ...
    def __init__(self, environment: str, table_name: str, run_migration: bool = False):
        ...
        self.failed_items_bucket = os.environ.get(
            "MIGRATION_FAILED_ITEMS_STORE_BUCKET_NAME"
        )
        if not self.failed_items_bucket:
            raise ValueError(
                "MIGRATION_FAILED_ITEMS_STORE_BUCKET_NAME environment variable is required"
            )
    ...
    def process_entries(
        ...
        """
        self.logger.info(
            f"Running '{label}' migration step on {len(entries := list(entries))} items"
        )
    ...
    def _process_single_entry(self, label, entry, update_fn, segment):
        ...
        try:
            ...
            self.logger.info(
                f"[{label}] Item {item_id} update fields: {updated_fields}"
            )
            ...
            self.logger.error(
                f"[{label}] Unrecoverable error for item {item_id} - segment {segment}: {e.message}"
            )
    ...
    def _handle_failed_items(self, label, segment, execution_id, failed_items):
        self.logger.error(
            f"'{label}' migration segment: {segment} completed with {len(failed_items)} errors."
        )
        ...
        try:
            ...
            self.logger.error(
                f"Error report saved to s3://{self.failed_items_bucket}/{error_report_key}"
            )
