...
def get_dynamodb_client(region):
    ...
    if region not in dynamodb_clients:
        dynamodb_clients[region] = boto3.client(
...
def validate_table_arn(event):
    ...
    if not table_arn.startswith("arn:aws:dynamodb:"):
        raise ValueError(
            "Invalid DynamoDB table ARN format - must start with 'arn:aws:dynamodb:'"
        )

    if ":table/" not in table_arn:
        raise ValueError(
            "Invalid DynamoDB table ARN format - missing ':table/' component"
        )
    ...
    try:
        ...
        if len(arn_parts) < 6:
            raise ValueError(
                "Invalid DynamoDB table ARN format - insufficient components"
            )
...
def validate_table_exists(table_name, region):
    ...
    try:
        ...
        if table_status not in ["ACTIVE", "UPDATING"]:
            raise ValueError(
                f"DynamoDB table '{table_name}' is not in ACTIVE status. Current status: {table_status}"
            )
        ...
        if error_code == "ResourceNotFoundException":
            ...
            raise ValueError(
                f"Access denied to DynamoDB table '{table_name}'. Check IAM permissions"
            )
        else:
...
def lambda_handler(event, context):
    ...
    try:
        ...
        extras = {
            "executionId": (
                execution_id if execution_id is not None else event.get("executionId")
            ),
            "totalSegments": (
                total_segments
                if total_segments is not None
                else event.get("totalSegments")
            ),
            "errorType": type(e).__name__,
        }
        logger.error(
            f"Exception in migration_dynamodb_segment_handler: {e}",
            extra=extras,
            exc_info=True,
        )
        raise
