...
from handlers.migration_dynamodb_segment_handler import (
    get_dynamodb_client,
    lambda_handler,
...
def mock_migration_service(mocker):
    ...
    mocked_class = mocker.patch(
        "handlers.migration_dynamodb_segment_handler.MigrationDynamoDBSegmentService"
    )
...
def mock_dynamodb_client(mocker):
    ...
    mocker.patch(
        "handlers.migration_dynamodb_segment_handler.get_dynamodb_client",
...
class TestValidateExecutionId:
    ...
    def test_validate_execution_id_invalid_values(self, execution_id):
        ...
        with pytest.raises(
            ValueError, match="Invalid or missing 'executionId' in event"
        ):
...
class TestValidateTotalSegments:
    ...
    @pytest.mark.parametrize(
        "total_segments,expected", [(4, 4), (1, 1), (1000, 1000), (4.0, 4), ("42", 42)]
    )
    ...
    def test_validate_total_segments_invalid_type(self, total_segments):
        ...
        with pytest.raises(
            ValueError,
            match="Invalid 'totalSegments' in event - must be a valid integer",
        ):
    ...
    def test_validate_total_segments_exceeds_maximum(self):
        ...
        with pytest.raises(
            ValueError, match="'totalSegments' exceeds maximum allowed value of 1000"
        ):
            validate_total_segments(event)
...
class TestValidateTableArn:
    ...
    def test_validate_table_arn_with_hyphenated_table_name(self):
        ...
        event = {
            "tableArn": "arn:aws:dynamodb:eu-west-2:123456789012:table/my-table-name"
        }
        table_name, region = validate_table_arn(event)
...
class TestValidateTableExists:
    ...
    def test_validate_table_exists_invalid_status(self, status, mock_dynamodb_client):
        ...
        with pytest.raises(
            ValueError, match=f"is not in ACTIVE status. Current status: {status}"
        ):
    ...
    def test_validate_table_exists_access_denied(self, mock_dynamodb_client):
        ...
        with pytest.raises(
            ValueError,
            match="Access denied to DynamoDB table 'MyTable'. Check IAM permissions",
        ):
            validate_table_exists("MyTable", "us-east-1")
    ...
    def test_validate_table_exists_other_client_error(self, mock_dynamodb_client):
        ...
        with pytest.raises(
            ValueError, match="Failed to validate DynamoDB table 'MyTable'"
        ):
            validate_table_exists("MyTable", "us-east-1")
...
class TestLambdaHandler:
    ...
    def test_lambda_handler_success(
        self, valid_event, mock_migration_service, mock_dynamodb_client
    ):
        ...
        expected_result = {
            ...
            "key": "stepfunctionconfig-execution-12345.json",
        }
    ...
    def test_lambda_handler_validation_error_execution_id(
        self, mock_migration_service, mock_dynamodb_client
    ):
    ...
    def test_lambda_handler_validation_error_total_segments(
        self, mock_migration_service, mock_dynamodb_client
    ):
    ...
    def test_lambda_handler_validation_error_table_arn(
        self, mock_migration_service, mock_dynamodb_client
    ):
    ...
    def test_lambda_handler_table_not_found(
        self, mock_migration_service, mock_dynamodb_client
    ):
    ...
    def test_lambda_handler_service_exception(
        self, valid_event, mock_migration_service, mock_dynamodb_client
    ):
    ...
    def test_lambda_handler_logging_on_validation_error(
        self, mocker, mock_dynamodb_client
    ):
    ...
    def test_lambda_handler_logging_on_service_error(
        self, valid_event, mock_migration_service, mocker, mock_dynamodb_client
    ):
        ...
        mock_migration_service.segment.side_effect = RuntimeError(
    ...
    def test_lambda_handler_logging_partial_validation_failure(
        self, mocker, mock_dynamodb_client
    ):
...
class TestGetDynamoDBClient:
    ...
    def test_get_dynamodb_client_creates_new_client(self, mocker):
        ...
        mock_boto_client = mocker.patch(
            "handlers.migration_dynamodb_segment_handler.boto3.client"
        )
        mock_client = mocker.MagicMock()
        ...
        mock_boto_client.assert_called_once_with(
            "dynamodb", region_name="us-east-1", config=mocker.ANY
        )
    ...
    def test_get_dynamodb_client_creates_separate_clients_per_region(self, mocker):
        ...
        mock_boto_client.assert_any_call(
            "dynamodb", region_name="eu-west-2", config=mocker.ANY
        )
...
class TestIntegration:
    ...
    @pytest.mark.parametrize(
        "execution_id,total_segments,expected_parsed_id,expected_total",
        [
            (
                "arn:aws:states:region:account:execution:machine:my-exec",
                4.0,
                "my-exec",
                4,
            ),
    ...
    def test_lambda_handler_full_flow(
        ...
        expected_result = {
            "bucket": "migration-bucket",
            "key": f"stepfunctionconfig-{expected_parsed_id}.json",
        }
        mock_migration_service.segment.return_value = expected_result
        ...
        mock_dynamodb_client.describe_table.assert_called_once_with(TableName="MyTable")
        mock_migration_service.segment.assert_called_once_with(
            expected_parsed_id, expected_total
        )
        assert result == expected_result
