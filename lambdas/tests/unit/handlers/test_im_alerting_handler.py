...
import pytest
from handlers.im_alerting_handler import lambda_handler
...
def mock_service_with_alarm_alert(mocker):
    ...
    mocked_class.return_value.message = LAMBDA_ALERT_MESSAGE
...
def test_handler_calls_handle_alarm_message_lambda_triggered_by_alarm_message(
    ...
    event = {"Records": [{"Sns": {"Message": json.dumps(LAMBDA_ALERT_MESSAGE)}}]}
