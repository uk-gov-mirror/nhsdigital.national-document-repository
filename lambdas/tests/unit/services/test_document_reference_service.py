...
def test_raises_no_available_document_when_no_docs_found(
    ...
    with pytest.raises(NoAvailableDocument):
        mock_lg_service.get_available_lloyd_george_record_for_patient(
            TEST_NHS_NUMBER
        )
...
def test_raises_file_upload_in_progress_when_document_uploading(
    ...
    assert (
        "in the process of being uploaded" in str(exc_info.value).lower()
    )

def test_returns_documents_when_uploaded_and_not_uploading(
    mock_lg_service, mocker
):
...
def test_updates_all_documents_with_different_ods_code(
    ...
    mock_lg_service.update_patient_ods_code(
