...
def mock_env_bucket_name(mocker):
    ...
    return mocker.patch.dict(
        "os.environ", {"MIGRATION_SEGMENT_BUCKET_NAME": "test-bucket"}
    )
...
def mock_s3_client(mocker):
    ...
    mock_client = mocker.patch(
        "services.migration_dynamodb_segment_service.boto3.client"
    )
...
def test_segment_success(service, mock_s3_client, mock_secrets_randbelow):
    ...
    expected_result = {"bucket": "test-bucket", "key": expected_key}
...
def test_segment_various_sizes(
    service, mock_s3_client, mock_secrets_randbelow, total_segments, expected_segments
):
...
def test_segment_special_characters_in_id(
    service, mock_s3_client, mock_secrets_randbelow, test_id, expected_key
):
...
def test_segment_error_handling(
    service, mock_s3_client, mock_secrets_randbelow, exception, exception_type
):
...
def test_segment_logging_on_error(
    service, mock_s3_client, mock_secrets_randbelow, mocker
):
...
def test_segment_environment_variable_missing(mocker):
    ...
    with pytest.raises(
        ValueError,
        match="MIGRATION_SEGMENT_BUCKET_NAME environment variable is required",
    ):
        MigrationDynamoDBSegmentService()


def test_segment_body_encoding_and_json(
    service, mock_s3_client, mock_secrets_randbelow
):
...
def test_segment_zero_segments_edge_case(
    service, mock_s3_client, mock_secrets_randbelow
):
...
def test_segment_actual_shuffle_behavior(service, mock_s3_client):
    ...
    assert any(
        result != sorted_segments for result in results
    ), "Shuffle should produce different orders"
