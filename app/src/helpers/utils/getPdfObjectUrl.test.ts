...
import { getPdfObjectUrl, getObjectUrl } from './getPdfObjectUrl';
...
describe('getPdfObjectUrl.ts', () => {
    ...
    describe('getPdfObjectUrl', () => {
        ...
        describe('Different URLs', () => {
            ...
            it('handles URLs with query parameters', async () => {
                //TODO Review tests
                const urlWithParams =
                    'https://cloudfront.example.com/file.pdf?token=abc&expires=123';
                const mockBlob = new Blob(['test pdf data'], { type: 'application/pdf' });
                mockedAxios.get.mockResolvedValue({ data: mockBlob });
            ...
            it('handles URLs with special characters', async () => {
                const specialUrl =
                    'https://cloudfront.example.com/files/patient%20record%20(1).pdf';
                const mockBlob = new Blob(['test pdf data'], { type: 'application/pdf' });
                mockedAxios.get.mockResolvedValue({ data: mockBlob });
    ...
    describe('getObjectUrl', () => {
        ...
        it('fetches blob from the provided URL', async () => {
            const mockBlob = new Blob(['test data'], { type: 'application/pdf' });
            mockedAxios.get.mockResolvedValue({ data: mockBlob });

            await getObjectUrl(testCloudFrontUrl);
