...
function get-cmd-to-run() {
  ...
  case $check in
    "whole-history")
      cmd="detect --source $dir --verbose --redact --no-banner"
      ;;
    "last-commit")
      cmd="detect --source $dir --verbose --redact --no-banner --log-opts -1"
      ;;
    "staged-changes")
      cmd="protect --source $dir --verbose --no-banner --staged"
      ;;
  esac
  # Include base line file if it exists
  if [[ -f "$dir/src/config/.gitleaks-baseline.json" ]]; then
    cmd="$cmd --baseline-path $dir/src/config/.gitleaks-baseline.json"
  fi
  # Include the config file
  cmd="$cmd --config $dir/src/config/gitleaks.toml"
...
function run-gitleaks-in-docker() {
  echo gitleaks not found locally, attempting to use Docker container
